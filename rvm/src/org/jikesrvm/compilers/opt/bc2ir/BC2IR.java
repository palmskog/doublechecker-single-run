      ...
      switch (code) {
        ...
        case JBC_invokevirtual: {
          ...
          if (ref.isMiranda()) {
            ...
            boolean unresolved = ref.needsDynamicLink(bcodes.getMethod());
            // AVD: Context: Compute context if the current method is non-application while the invoked method is an 
            // application method.
            ...
            if (type != null && type.isResolved()) {
              if (type.isClassType()) {
                ...
                if (target == null || type != target.getDeclaringClass()) {
                  vmeth = ClassLoaderProxy.lookupMethod(type.asClass(), ref, context); // AVD: Changed parameter
                }
                if (vmeth != null) {
          ...
          if (requiresImplementsTest && resolvedMethod == null) {
            ...
            Instruction callCheck =
              Call.create3(CALL,
                           ...
                           receiver.copy(),
                           IRTools.IC(context)); // AVD: Changed parameter
...
////REPLACE LOCALS ON STACK.
  ...
  private InlineDecision shouldInline(Instruction call, boolean isExtant, int realBCI) {
    ...
    // AVD: Don't allow inlining a Tx into a non-Tx
  ...
  private boolean maybeInlineMethod(InlineDecision inlDec, Instruction callSite) {
    ...
    // AVD: Assert that a Tx is not inlined into a non-Tx
    if (VM.runningVM && VM.VerifyAssertions) {
      if (gc.method.getMemberRef().asMethodReference().isNonAtomic 
