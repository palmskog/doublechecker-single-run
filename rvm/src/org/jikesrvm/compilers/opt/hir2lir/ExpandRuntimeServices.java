...
public final class ExpandRuntimeServices extends CompilerPhase {
  ...
  public void perform(IR ir) {
      ...
      switch (opcode) {
        ...
        case MONITORENTER_opcode: {
          ...
          if (refType != null && !refType.getThinLockOffset().isMax()) {
            ...
            if (Octet.shouldInstrumentMethod(inst.position.getMethod())) {
              target = Entrypoints.inlineLockMethod;
            } else {
              target = Entrypoints.inlineLockMethodWithoutAVDInstrumentation;
            }
            Call.mutate2(inst,
                         CALL,
                         null,
            ...
            RVMMethod target;
            if (Octet.shouldInstrumentMethod(inst.position.getMethod())) {
              target = Entrypoints.lockMethod;
            } else {
              target = Entrypoints.lockMethodWithoutAVDInstrumentation;
            }
            Call.mutate1(inst,
                         CALL,
                         null,
        ...
        case MONITOREXIT_opcode: {
          ...
          if (refType != null && !refType.getThinLockOffset().isMax()) {
            ...
            RVMMethod target;
            if (Octet.shouldInstrumentMethod(inst.position.getMethod())) {
              target = Entrypoints.inlineUnlockMethod;
            } else {
              target = Entrypoints.inlineUnlockMethodWithoutAVDInstrumentation;
            }
            Call.mutate2(inst,
                         CALL,
                         null,
            ...
            RVMMethod target;
            if (Octet.shouldInstrumentMethod(inst.position.getMethod())) {
              target = Entrypoints.unlockMethod;
            } else {
              target = Entrypoints.unlockMethodWithoutAVDInstrumentation;
            }
            Call.mutate1(inst,
                         CALL,
                         null,
    ...
    // AVD: Visualize CFG after inlining of barriers
    if(ir.options.VISUALIZE_CFG) {     
      CFGVisualization  cfg = new CFGVisualization(ir, "endofExpandRuntimeServices");
      cfg.visualizeCFG(ir);
    }
