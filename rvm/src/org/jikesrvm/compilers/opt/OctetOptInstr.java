...
public class OctetOptInstr extends CompilerPhase {
  ...
  // AVD: Made this protected so that it is visible in derived classes
...
//    // AVD: Debugging inlining performance
//    if(ir.options.VISUALIZE_CFG) {
//      CFGVisualization cfgAfter = new CFGVisualization(ir, "afterOctetBeforeInlining");
...
//    }
    ...
    if (ir.options.VISUALIZE_CFG) {
      // AVD: This method needs to be invoked to bring sanity into the numbering of basic blocks.
      // Otherwise, the basic block numbering may get messed up due to barrier inlining, although that does
      // not affect correctness. The control flow is not affected, but CFG visualization is messed up.
  ...
  void insertBarrier(Instruction barrierCall, Instruction inst, boolean isRead, Operand refOperand, RVMField field, boolean isResolved, HashSetRVM<Instruction> callsToInline, IR ir) {
    ...
    if (inliningType == InliningType.INSERT_IR_INSTS &&
      ...
      // AVD: Avoiding inlining barrier calls for DaCapo benchmarks. The compilation increases manifold, and the performance
      // of the DaCapo benchmarks degrade. On the other hand, inlining barriers provide significant performance boost for 
      // ETHZ and Grande benchmarks.
      if (!AVD.isAVDEnabled() // Non-AVD configurations
          || AVD.bench.getId() != BenchmarkInfo.MONTECARLO /*Don't inline barriers for Montecarlo*/) {
        ...
        if (inliningType == InliningType.JIKES_INLINER &&
            ...
            isResolved) {
          if (VM.VerifyAssertions) { VM._assert(AVD.inlineBarriers()); }
          // AVD: Don't inline for Xalan9, it is very sensitive to the compilation time overheads
          if (AVD.isAVDEnabled() && AVD.bench.getId() != BenchmarkInfo.XALAN9) {
   ...
   */
  protected void inline(Instruction inst, IR ir, boolean noCalleeExceptions) { // AVD: Changed visibility
    ...
    try {
      InlineDecision inlDec =
          InlineDecision.YES(Call.getMethod(inst).getTarget(), "Inlining of Octet barriers");
