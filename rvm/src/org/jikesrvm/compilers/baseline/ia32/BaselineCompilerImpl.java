...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants, SizeConstants {
  ...
  protected final void emit_iaload() {
    ...
    // AVD: Conditional instrumentation of array accesses
    if (AVD.instrumentArrays()) {
  ...
  protected final void emit_ireturn() {
    ...
    // AVD: Insert instrumentation at method return
    if (Octet.shouldInstrumentMethod(method) && (AVD.methodsAsTransactions() || method.isSynchronized() && AVD.syncBlocksAsTransactions())) {
      if (Context.isTRANSContext(method.getStaticContext()) && Context.isNONTRANSContext(method.getResolvedContext())) {
        AVDBaselineInstr.insertInstrumentationAtTransactionEnd(method, biStart, asm, octetBaselineInstr, method.getId());
      }
    }
  ...
  // AVD: Context: Added a pre-computed context
  @Override
  ...
  protected final void emit_invokeinterface(MethodReference methodRef) {
    ...
    if (VM.BuildForIMTInterfaceInvocation) {
      ...
      // AVD: Context: Interface invocation for non-application methods can only have VM_CONTEXT
  ...
  protected final void emit_monitorenter() {
    ...
    // AVD: Instrumentation at the beginning of transaction/synchronized block    
    // We can have a configuration where both methods and sync blocks are enabled
    if (Octet.shouldInstrumentMethod(method) && AVD.syncBlocksAsTransactions()) {
      if (VM.VerifyAssertions) { VM._assert(NOT_REACHED); }
      AVDBaselineInstr.insertInstrumentationAtTransactionBegin(method, biStart, asm, octetBaselineInstr, method.getId());
    }   
    ...
    genNullCheck(asm, T0);
    genParameterRegisterLoad(asm, 1);      // pass 1 parameter word
    // AVD: Decide which lock method to use
    if (Octet.shouldInstrumentMethod(method)) {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.lockMethod.getOffset()));
    } else {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.lockMethodWithoutAVDInstrumentation.getOffset()));
    }
  }
  ...
  protected final void emit_monitorexit() {
    genParameterRegisterLoad(asm, 1);          // pass 1 parameter word
    // AVD: Decide which unlock method to use
    if (Octet.shouldInstrumentMethod(method)) {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.unlockMethod.getOffset()));
    } else {
      asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.unlockMethodWithoutAVDInstrumentation.getOffset()));
    }

    // AVD: Instrumentation at the end of transaction/synchronized block  
    // We can have a configuration where both methods and sync blocks are enabled
    if (Octet.shouldInstrumentMethod(method) && AVD.syncBlocksAsTransactions()) {     
      if (VM.VerifyAssertions) { VM._assert(NOT_REACHED); }
      AVDBaselineInstr.insertInstrumentationAtTransactionEnd(method, biStart, asm, octetBaselineInstr, method.getId());      
  ...
  private void genPrologue() {
    ...
    if (klass.hasBridgeFromNativeAnnotation()) {
       ...
       */
      // AVD: Stack overflow: Insert stack overflow checks for all uninterruptible methods if assertions are turned on
      ...
      // AVD: Instrumentation at the beginning of a method/transaction
      ...
      if (Octet.shouldInstrumentMethod(method) && (AVD.methodsAsTransactions() || method.isSynchronized() && AVD.syncBlocksAsTransactions())) {
        ...
        if (AVD.checkMethodContextAtProlog() && Context.isApplicationPrefix(method.getDeclaringClass().getTypeRef())) {
          asm.emitCALL_Abs(Magic.getTocPointer().plus(Entrypoints.avdcheckMethodContextAtPrologMethod.getOffset()));
        }
      }
