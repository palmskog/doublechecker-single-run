...
public abstract class BaselineExceptionDeliverer extends ExceptionDeliverer implements BaselineConstants {
  ...
  public void unwindStackFrame(CompiledMethod compiledMethod, ArchitectureSpecific.Registers registers) {
      ...
      if (instr.sGT(lockOffset)) { // we actually have the lock, so must unlock it.
        ...
        if (ObjectModel.holdsLock(lock, RVMThread.getCurrentThread())) {
          ...
          if (Octet.shouldInstrumentMethod(method)) {
            ...
            ObjectModel.genericUnlockWithoutAVDInstrumentation(lock);
          }
        }
      }
    }
    ...
    // AVD: Decrement the transaction depth for the current thread
    RVMMethod rMethod = compiledMethod.getMethod();
    if (!rMethod.isNative()) {
      if (Octet.shouldInstrumentMethod(rMethod) && (AVD.methodsAsTransactions() || rMethod.isSynchronized() && AVD.syncBlocksAsTransactions())) {
        if (Context.isTRANSContext(rMethod.getStaticContext()) && Context.isNONTRANSContext(rMethod.getResolvedContext())) {
          AVDAnalysis.handleExceptionDuringStackUnwinding(rMethod);
        }
      }
    }
