...
public final class RVMField extends RVMMember {
  ...
  /**AVD: the offset of direct per-field metadata (currently for static fields only) */
  private Offset avdMetadataOffset = Offset.fromIntSignExtend(NO_OFFSET);
  ...
  // AVD: Helper methods for metadata offset
  
  @Uninterruptible
  @Inline
  public final boolean hasAVDMetadataOffset() {
    if (!AVD.addPerFieldMetadata()) {
      if (VM.VerifyAssertions) { VM._assert(avdMetadataOffset.toInt() == NO_OFFSET); }      
      return false;
    } else {
      return avdMetadataOffset.toInt() != NO_OFFSET;
    }
  }

  @Uninterruptible
  @Inline
  public final Offset getAVDMetadataOffset() {
    if (VM.VerifyAssertions) { VM._assert(hasAVDMetadataOffset()); }
    return avdMetadataOffset;
  }

  @Uninterruptible
  @Inline
  public final void setAVDMetadataOffset(Offset offset) {
    if (VM.VerifyAssertions) { VM._assert(!hasAVDMetadataOffset()); }
    avdMetadataOffset = offset;
  }
