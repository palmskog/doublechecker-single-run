...
// AVD: Context: Changed this class extensively to support two static contexts (transactional and non-transactional) 
...
public final class Context implements Constants {
  ...
  /* AVD: There is a complicated relationship between few packages. For example, gnu/javax packages extend
  ...
  {"Ljava/".getBytes(),
   ...
   //"Ljavax/".getBytes(), // AVD: Many benchmarks like sunflow9, xalan9 make use of this package
   //"Lorg/xml/".getBytes(),
  };
  ...
  static boolean isMatch(RVMMethod rvmMethod, int context, boolean resolve) {
    if (rvmMethod.getMemberRef().asMethodReference().hasMultipleResolvedContexts()) { // Application methods
      ...
      // AVD: There could be application classes that implement interfaces, but don't override an abstract method. In
