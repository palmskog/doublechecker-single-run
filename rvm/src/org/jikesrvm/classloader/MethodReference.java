...
// AVD: Context: Changed this class to support two static contexts for application methods (possibly 3 resolved contexts), 
...
public final class MethodReference extends MemberReference {
  ...
  // AVD: Adding a flag to track whether this method is supposed to be non-atomic
  ...
  // AVD: Context: References to different versions of RVMMethod depending on the resolved context
  private RVMMethod resolvedMemberVM;
  private RVMMethod resolvedMemberTrans;
  private RVMMethod resolvedMemberNonTrans;

  // AVD: This now is meaningful only for application methods
  ...
  MethodReference(TypeReference tr, Atom mn, Atom d, int id) {
    ...
    // AVD: Check and store the fact whether the given method is supposed to be non-atomic
    Atom at = AVD.constructMethodSignature(this);
    if (AVD.notTransactions.contains(at)) {
      isNonAtomic = true;
    } else {
      isNonAtomic = false;
    }
