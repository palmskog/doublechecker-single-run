...
public final class RVMClass extends RVMType implements Constants, ClassLoaderConstants {
  ...
  public RVMMethod findDeclaredMethod(Atom methodName, Atom methodDescriptor, int context) {
    // AVD: Context: Library class (javax.*) can extend application classes (gnu.java.*), example hedc.
    ...
    for (RVMMethod method : declaredMethods) {
      if (method.getName() == methodName && method.getDescriptor() == methodDescriptor) {
        foundSomething = true;
        if (Context.isMatch(method, context, /* resolve = */ false)) { // AVD: Context: Added a parameter
          return method;
        }
      }
    }
  ...
  public RVMMethod findMainMethod() {
    ...
    // AVD: Context: main() should be non-atomic
  ...
  public RVMMethod findStaticMethod(Atom memberName, Atom memberDescriptor, int context) {
    ...
    for (RVMMethod method : getStaticMethods()) {
      if (method.getName() == memberName && method.getDescriptor() == memberDescriptor &&
          Context.isMatch(method, context, false)) { // AVD: Context: Added a parameter
        return method;
      }
    }
    return null;
  ...
  // AVD: Context: Modified the resolve() method to support 2 static contexts and 3 resolved application contexts.
  ...
  public synchronized void resolve() {
    ...
    //
    {
      ...
      for (int j = 0, m = virtualMethods.size(); j < m; j++) {
        ...
        if (overridingMethod != null && !hasContextMatch) {
          if (Context.isApplicationPrefix(this.getTypeRef())) { // AVD: Context: Clone only for application methods
    ...
    for (RVMField field : staticFields) {
      ...
      if (Octet.getConfig().addHeaderWord() &&
        ...
        // AVD: Add per-field metadata for static, non-final fields. Initialize metadata.
        if (AVD.addPerFieldMetadata()) {
          Offset offset = Statics.allocateNumericSlot(BYTES_IN_WORD, true);
          // Verify if the offset can ever be an odd number, since RVMMember.NO_OFFSET is
          // initialized to Short.MIN_VALUE
          if (VM.VerifyAssertions && (offset.toInt() % 2 != 0)) {
            VM.sysWriteln("Value of offset:", offset.toInt());
            VM._assert(NOT_REACHED);
          }
          field.setAVDMetadataOffset(offset);
          // Fields in Java should already be initialized to zero
          //Statics.setSlotContents(offset, Word.zero());
        }
      }
    ...
    // AVD: Context: Verify that non-application methods should have only one VM_CONTEXT
  ...
  private void printMethods() {
    ...
    for (RVMMethod m : virtualMethods) {
      VM.sysWrite(AVD.constructMethodSignature(m));
...
//    }
  ...
  public synchronized void initialize() throws ExceptionInInitializerError {
    ...
    if (classInitializerMethod != null) {
      ...
      // AVD: Context: Need to invoke the correct version of class initializer irrespective of where <clinit> gets
