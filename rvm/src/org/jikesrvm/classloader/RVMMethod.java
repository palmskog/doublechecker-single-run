...
public abstract class RVMMethod extends RVMMember implements BytecodeConstants {
  ...
  protected RVMMethod(TypeReference declaringClass, MemberReference memRef, short modifiers,
    ...
    // AVD: Context: Initialize static context for application methods 
  ...
  /** AVD: Context: Compute static context only for application methods. Logically, the static context of a method 
  ...
  public final synchronized CodeArray getCurrentEntryCodeArray() {
    ...
    if (isCompiled()) {
      ...
      if (!isStatic() && !isObjectInitializer() && !isPrivate()) {
        // AVD: Context: Walk the stack to find out which RVMMethod version to use depending on the context
        ...
        if (declaringClass.isJavaLangObjectType() ||
            declaringClass.getSuperClass().findVirtualMethod(getName(), getDescriptor(), /*getResolvedContext()*/ context) == null) { // AVD: Changed parameter
