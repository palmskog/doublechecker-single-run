...
public class VM extends Properties implements Constants, ExitStatus {
  ...
  public static void boot() {
    ...
    // AVD: Adding a non-standard argument to identify benchmark category
    benchmarkCategory = BootRecord.the_boot_record.benchmarkCategory;
    if (VM.VerifyAssertions) { VM._assert(benchmarkCategory >= 0); }
    if (verboseBoot >= 1) VM.sysWriteln("AVD: Benchmark category:", benchmarkCategory);    
  ...
  private static void finishBooting() {
    ...
    // AVD: Hack to prevent running slowpath hooks during booting
    AVDAnalysis.isVMFullyBooted = true;
  ...
  // AVD: Added this to avoid printing hex values
  @NoInline
  public static void sysWriteln(long l, boolean hexToo) {
    swLock();
    write(l, hexToo);
    writeln();
    swUnlock();    
  ...
  @NoInline
  public static void sysWrite(String s, Atom a) {
    swLock();
    write(s);
    write(a);
    swUnlock();
