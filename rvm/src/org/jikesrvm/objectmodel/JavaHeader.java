...
public class JavaHeader implements JavaHeaderConstants {
  ...
  // AVD: Changes for lock acquire()/release()
  /**
   * Generic lock with AVD instrumentation
   */
  ...
  public static void genericLock(Object o) {
    ...
    if (AVD.trackSynchronizationPrimitives()) { // No state change required if AVD is not enabled
      StateTransfers.read(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, STATUS_OFFSET.toInt(), -1);
      if (AVD.logRdWrAccesses()) {
        RVMThread currentThread = RVMThread.getCurrentThread();
        if (currentThread.isOctetThread()) {
          AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), STATUS_OFFSET.toInt(), 
              AVD.LOCK_READ/*, -1*/);
  ...
  /**
   * Generic lock without AVD instrumentation
   */
  @Unpreemptible("Become another thread when lock is contended, don't preempt in other cases")
  public static void genericLockWithoutAVDInstrumentation(Object o) {
    ThinLock.lock(o, STATUS_OFFSET);
  }

  /**
   * Generic unlock with AVD instrumentation
   */
  @Unpreemptible("No interruption unless of exceptions")
  public static void genericUnlock(Object o) {
    if (AVD.trackSynchronizationPrimitives()) { // No state change required if AVD is not enabled
      StateTransfers.write(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, STATUS_OFFSET.toInt(), -1);
      if (AVD.logRdWrAccesses()) {
        RVMThread currentThread = RVMThread.getCurrentThread();
        if (currentThread.isOctetThread()) {
          AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), STATUS_OFFSET.toInt(), 
  ...
  /**
   * Generic unlock without AVD instrumentation
   */
  @Unpreemptible("No interruption unless of exceptions")
  public static void genericUnlockWithoutAVDInstrumentation(Object o) {
    ThinLock.unlock(o, STATUS_OFFSET);
  }
