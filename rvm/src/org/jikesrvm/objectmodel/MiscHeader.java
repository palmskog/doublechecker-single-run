...
public final class MiscHeader implements Constants, MiscHeaderConstants {
  ...
  /** AVD: Offset for a word which will be used for array metadata */
  public static final Offset AVD_ARRAY_OFFSET = OCTET_OFFSET.plus(OCTET_HEADER_BYTES);
  ...
  public static void initializeHeader(Object obj, TIB tib, int size, boolean isScalar) {
    ...
    // AVD: No need to zero the metadata word, since the JMM guarantees that
//    // AVD: Initialize the metadata for array 
//    if (AVD.instrumentArrays()) {
//      ObjectReference.fromObject(obj).toAddress().store(Word.zero(), AVD_ARRAY_OFFSET);
//    }
  ...
  public static void initializeHeader(BootImageInterface bootImage, Address ref, TIB tib, int size,
    ...
    // AVD: No need to zero the metadata word, since the JMM guarantees that
//    // AVD: initialize boot image objects
//    if (AVD.instrumentArrays()) {
//      bootImage.setAddressWord(ref.plus(AVD_ARRAY_OFFSET), Word.zero(), false, false);
//    }
