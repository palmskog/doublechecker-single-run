...
public abstract class FieldLayout implements SizeConstants {
  ...
  */
  ...
  /** AVD: make room for per-field metadata */
  private boolean layoutFieldMetadata(FieldLayoutContext layout, RVMClass klass, RVMField field) {
    if (AVD.shouldAddPerFieldAVDMetadata(field)) {
      if (!field.hasAVDMetadataOffset()) {
        int numFields = AVD.getNumFields(field); // Currently is one for AVD
        Offset metadataOffset;
        if (numFields > 1) {
          metadataOffset = Offset.fromIntSignExtend(layout.nextOffsetUnaligned(AVD.FIELD_SIZE * numFields));
        } else {
          metadataOffset = Offset.fromIntSignExtend(layout.nextOffset(AVD.FIELD_SIZE, true));
        }
        setAVDMetadataOffset(klass, field, metadataOffset);
        return true;
      }
    }
    return false;
  ...
  /** AVD: Set metadata offset */
  protected void setAVDMetadataOffset(RVMClass klass, RVMField field, Offset offset) {
    ...
    field.setAVDMetadataOffset(metadataOffset);
  }
