...
public final class ThinLock implements ThinLockConstants {
  ...
  // AVD: Lock acquire() with instrumentation
  @Inline
  @NoNullCheck
  @Unpreemptible
  public static void inlineLock(Object o, Offset lockOffset) {
    ...
    if (AVD.trackSynchronizationPrimitives()) { // No state change required if AVD is not enabled
      StateTransfers.read(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, lockOffset.toInt(), -1);
      if (AVD.logRdWrAccesses()) {
        RVMThread currentThread = RVMThread.getCurrentThread();
        if (currentThread.isOctetThread()) {
          AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), lockOffset.toInt(),
              AVD.LOCK_READ/*, -1*/);
  ...
  // AVD: Lock acquire() without instrumentation
  @Inline
  @NoNullCheck
  @Unpreemptible
  public static void inlineLockWithoutAVDInstrumentation(Object o, Offset lockOffset) {
    inlineLockHelper(o, lockOffset);
  }
  ...
  // AVD: Lock release() with AVD instrumentation
  @Inline
  @NoNullCheck
  @Unpreemptible
  public static void inlineUnlock(Object o, Offset lockOffset) {
    if (AVD.trackSynchronizationPrimitives()) { // No state change required if AVD is not enabled
      StateTransfers.write(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, lockOffset.toInt(), -1);
      if (AVD.logRdWrAccesses()) {
        RVMThread currentThread = RVMThread.getCurrentThread();
        if (currentThread.isOctetThread()) {
          AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), lockOffset.toInt(),
  ...
  // AVD: Lock release() without AVD instrumentation
  @Inline
  @NoNullCheck
  @Unpreemptible
  public static void inlineUnlockWithoutAVDInstrumentation(Object o, Offset lockOffset) {
    inlineUnlockHelper(o, lockOffset);
  }  
