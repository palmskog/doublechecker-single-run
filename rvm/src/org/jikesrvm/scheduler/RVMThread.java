...
import org.jikesrvm.atomicity.AVDDebugNode;
import org.jikesrvm.atomicity.AVDHashTable;
import org.jikesrvm.atomicity.AVDLogger;
import org.jikesrvm.atomicity.AVDMetadataHelper;
import org.jikesrvm.atomicity.AVDNode;
import org.jikesrvm.atomicity.AVDStats;
...
import org.jikesrvm.octet.StateTransfers;
...
public final class RVMThread extends ThreadContext implements Constants {
   ...
   */
  // AVD: Making this public to solve the ThreadQueue::enqueue() problem
  @Untraced
  public volatile ThreadQueue queuedOn;
...
//  public int padding;
  ...
  // AVD: The assertion that takeYieldpoint and octetRequests are on the same cache line fails
  // probably in AVD Phase 2 thread. 
//  private int dummy;
  ...
  // AVD: Thread-local variable declarations 
  ...
  public int numberOfRegularTransactions;
  /** Keep track of the last node in the graph, which also doubles up as the current transaction */
  public AVDNode currentTransaction;  
  /** Reference to the last transaction that did a conflicting transition for an exclusive read access */
  @Entrypoint
  @Untraced
  public AVDNode lastTransToPerformWrExToRdEx;
  public int lastLogIndexToPerformWrExToRdEx;
  public SpinLock lockForLastWrExToRdEx; // Protect updates to the two "last WrExToRdEx" variables
  // AVD: TODO: Is it necessary to treat this as a weak reference? It can be made local.
  public AVDNode penultimateTransToUpdateGlobalRdShCounter;
  public int logIndexForPenultimateTransToUpdateGlobalRdShCounter;
   /** This is a hash map from field address -> object reference. */
  @Entrypoint
  public WordArray accessInfoHashTable;
  /** Monitor to serialize cycle detection */
  public static Monitor phase1SCCLock;
  /** A linear array to store hash table slots that are used */
  public WordArray linearArray;
  public int indexOfLinearArray;
  /** Indicates whether a boundary point needs to be created. */
  volatile public boolean createBoundaryPoint; // Is shared
  /** Store transaction reference of remote thread */
  public AVDNode[] txArrayForBPCreation;
  /** Store current log index for the corresponding transaction reference of remote thread */
  public int[] logIndexArrayForBPCreation;
  /** Prevent creating multiple unary transactions as part of boundary point creation in both the implicit
   * and explicit protocol */
  public boolean unaryTransactionAlreadyCreated; // All accesses are thread-local
  /** Thread-local counter to keep track of number of boundary points created per-transaction. This should be
   * synonymous with hash table clears */
  public int bpCount;
  
  // AVD: TODO: Optimize accesses by storing the current address instead/along with the array index
  /** This is an array of references to word arrays, which constitute the log buffer. This reference would get traced, during
   *  GC, but the constituent references won't get traced. */
  public AddressArray logBufferReferenceArray;
  /** Keep track of the index of the current reference in the log buffer */
  public int logBufferReferenceArrayIndex;
  /** Logical index within one particular word array */
  public int wordArrayBufferIndex;
  /** Point to the current word array being used */
  public ObjectReference currentWordArrayStart;
  /** Point to the current empty word array slot */
  public Address currentWordArraySlot;
  //public boolean slowPathTaken; // Variable for debugging, set whenever the slow path is taken
  ...
  // AVD: Enabled this so that object references from RVMThread are traced and are not collected
   ...
   */
  // AVD: Changes to delay termination till Phase 2 is completed
   ...
   */
  // AVD: Changes to delay termination till Phase 2 is completed
  protected static int numActiveDaemons;
  ...
  public static void boot() {
    ...
    // AVD: Monitor to serialize cycle detection
    phase1SCCLock = new Monitor();
    ...
    if (!neverKillThreads) {
      restart: while(true) {
        ...
        for (int i = 0; i < aboutToTerminateN; ++i) {
          ...
          // AVD: Don't remove "terminated" threads from the slot if they are application
          // threads since Phase 2 might still be processing them. In that case, GC needs
          // to be able to scan the thread log buffer references array.
          if (t.getExecStatus() == TERMINATED && !t.isOctetThread()) {
            //VM.sysWriteln("Thread is being removed from slot ", t.octetThreadID);
  ...
  public RVMThread(byte[] stack, Thread thread, String name, boolean daemon, SystemThread systemThread, int priority) {
    ...
    // AVD: Initialize variables
    isInTransaction = false;
    numberOfNodes = AVD.START_TRANSACTION_ID;
    numberOfRegularTransactions = 0;
    lastTransToPerformWrExToRdEx = null;
    lastLogIndexToPerformWrExToRdEx = 0;
    lockForLastWrExToRdEx = new SpinLock();
    penultimateTransToUpdateGlobalRdShCounter = null;
    logIndexForPenultimateTransToUpdateGlobalRdShCounter = 0;
    createBoundaryPoint = false;
    indexOfLinearArray = 0;
    unaryTransactionAlreadyCreated = false;
    // It shouldn't matter that we pass true for read. Also note that 'this' is different from doing a getCurrentThread()
    bpCount = /*AVD.INITIAL_BP_COUNT*/ AVDMetadataHelper.createMetadata(this, AVD.INITIAL_BP_COUNT, true).toInt();
    logBufferReferenceArrayIndex = 0;
    wordArrayBufferIndex = 0;
    if (isOctetThread()) {
      txArrayForBPCreation = new AVDNode[MAX_THREADS];
      logIndexArrayForBPCreation = new int[MAX_THREADS];
      accessInfoHashTable = (AVD.recordAccessInfoInHashTable()) ? WordArray.create(AVDHashTable.INITIAL_HASHTABLE_SIZE) : null;
      // Ctors are always interruptible
      //MemoryManager.startAllocatingInUninterruptibleCode();
      currentTransaction = (!AVD.useDebugNodes()) ? new AVDNode(this, /*isUnary = */ true) 
                                                  : new AVDDebugNode(this, /*isUnary = */ true);
      //MemoryManager.stopAllocatingInUninterruptibleCode();
      
      linearArray = (AVD.recordAccessInfoInHashTable() && AVD.useLinearArrayOfIndices()) 
          ? WordArray.create(AVDHashTable.INITIAL_HASHTABLE_SIZE) : null;
      if (AVD.logRdWrAccesses()) {
        // This gets allocated in the nursery, which will later move
        logBufferReferenceArray = AddressArray.create(ReadWriteLog.INITIAL_LOG_BUFFER_REFERENCE_SIZE);
        //VM.sysWrite("Thread:", octetThreadID);
        //VM.sysWriteln(" Log buffer address:", ObjectReference.fromObject(logBufferReferenceArray).toAddress());
        WordArray temp = (WordArray) MemoryManager.createAVDSpace(ReadWriteLog.LOG_BUFFER_SLOTS * ReadWriteLog.NUM_WORDS_IN_CELL);
        if (VM.VerifyAssertions) { VM._assert(MemoryManager.validRef(ObjectReference.fromObject(temp))); }
        //VM.sysWriteln("Address of word array:", ObjectReference.fromObject(temp).toAddress());
        // This is not a reference from old space to nursery space, so generational barriers are not required.
        logBufferReferenceArray.set(logBufferReferenceArrayIndex, ObjectReference.fromObject(temp).toAddress());
        currentWordArraySlot = logBufferReferenceArray.get(logBufferReferenceArrayIndex);
        currentWordArrayStart = currentWordArraySlot.toObjectReference();
      }
    } else {
      txArrayForBPCreation = null;
      logIndexArrayForBPCreation = null;
      currentTransaction = null;
      accessInfoHashTable = null;
      linearArray = null;
      logBufferReferenceArray = null;
      currentWordArrayStart = ObjectReference.nullReference();
      currentWordArraySlot = Address.zero();
    }
  ...
  /** AVD: Check whether we are already in a transaction. */
  @Inline
  ...
  private void handleDebugRequestForThread() {
    ...
    if (!isAboutToTerminate) {
      ...
      if (isInJava()) {
        ...
        // AVD: Debugging why Jikes gets stuck
  ...
  public void terminate() {
    ...
    // AVD: Changes to delay termination
    if (traceAcct)
      VM.sysWriteln("active = ", numActiveThreads, ", daemons = ", numActiveDaemons);
    
    // AVD: Changes to delay termination till AVD Phase 2 is completed
    if ((numActiveDaemons == numActiveThreads) && ((VM.mainThread != null) && VM.mainThread.completed)) {
    ...
    if (terminateSystem) {
      ...
      // AVD: Changes to add dummy end node for the last thread
      // AVD: LATER: In Octet, for the last thread, if the RVM termination decision has been taken,
      // then Octet::handleThreadTerminationLate() is not called for that thread
      if (AVD.isAVDEnabled()) {
        AVDAnalysis.handleThreadTerminationHelper();
      }
   ...
   */
  @Interruptible
  protected void callSystemExit(final int exitStatus) { // AVD: Changing visibility
  ...
  private void terminateUnpreemptible() {
    ...
    if (isOctetThread()) {
      ...
      // AVD: We maintain an array of RVMThreads, so that the object references in RVMThreads get traced
      // But that would increase the reduce space overhead, therefore we are resetting the stack to null
      getCurrentThread().stack = null;
    }
  ...
  void waitImpl(Object o, boolean hasTimeout, long whenWakeupNanos) {
    ...
    if (asyncThrowable != null) {
      ...
      // AVD: Tracking Thread::wait()
      if (AVD.trackThreadSynchronizationPrimitives()) { // No state change required if AVD is not enabled        
        StateTransfers.read(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, JavaHeader.STATUS_OFFSET.toInt(), -1);
        if (AVD.logRdWrAccesses()) {
          RVMThread currentThread = RVMThread.getCurrentThread();
          if (currentThread.isOctetThread()) {
            AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), 
                JavaHeader.STATUS_OFFSET.toInt(), AVD.LOCK_READ/*, -2*/);
            if (AVD.recordAVDStats()) {
              AVDStats.numTotalPhase1Instructions.inc(1L);
              if (currentThread.currentTransaction.isUnaryTrans) {
                if (VM.VerifyAssertions) { VM._assert(!inTransaction()); }
  ...
  public static void notify(Object o) {
    ...
    // AVD: Tracking Thread::notify()
    if (AVD.trackThreadSynchronizationPrimitives()) { // No state change required if AVD is not enabled
      StateTransfers.write(ObjectReference.fromObject(o).toAddress(), MiscHeader.OCTET_OFFSET, JavaHeader.STATUS_OFFSET.toInt(), -1);
      if (AVD.logRdWrAccesses()) {
        RVMThread currentThread = RVMThread.getCurrentThread();
        if (currentThread.isOctetThread()) {
          AVDLogger.logAccessInReadWriteSet(currentThread, ObjectReference.fromObject(o).toAddress(), 
              JavaHeader.STATUS_OFFSET.toInt(), AVD.LOCK_WRITE/*, -2*/);
          if (AVD.recordAVDStats()) {
            AVDStats.numTotalPhase1Instructions.inc(1L);
            if (currentThread.currentTransaction.isUnaryTrans) {
              if (VM.VerifyAssertions) { VM._assert(!getCurrentThread().inTransaction()); }
              AVDStats.numBarrierCallsFromUnaryTx.inc(1L);
            } else {
              if (VM.VerifyAssertions) { VM._assert(getCurrentThread().inTransaction()); }
              AVDStats.numBarrierCallsFromRegularTx.inc(1L);
  ...
  public void join(long ms, int ns) throws InterruptedException {
    ...
    // AVD: We do not track join() since it calls wait()
  ...
  static void tracebackWithoutLock() {
    if (VM.runningVM) {
      // AVD: Printing additional debug information
  ...
  public static void dumpStack() {
    if (VM.runningVM) {
      ...
      // AVD: Printing additional debugging information
      ...
      VM.sysWriteln(); // AVD: Added this to format the output better
    } else {
                ...
                if (VM.BuildForOptCompiler && compiledMethod.getCompilerType() == CompiledMethod.OPT) {
                  ...
                  if (iei >= 0) {
                    ...
                    for (; iei >= 0; iei = OptEncodedCallSiteTree.getParent(iei, inlineEncoding)) {
                      ...
                      // AVD: There is a crash while dumping the stack
  ...
  public static void dumpVirtualMachine() {
    ...
    for (int i = 0; i < numThreads; ++i) {
      ...
      // AVD: Adding new line for better formatting
      VM.sysWriteln();
    }
    getCurrentThread().enableYieldpoints();
    Monitor.unlock(b, dumpLock);
  ...
  // AVD: Adding debug methods to dump only octet threads
  ...
  public static void dumpOctetThread(RVMThread t) {
    if (VM.VerifyAssertions) { VM._assert(t.isOctetThread()); }
    t.dump();
    if (t.contextRegisters != null && !t.ignoreHandshakesAndGC())
      dumpStack(t.contextRegisters.getInnermostFramePointer());
    VM.sysWriteln();
  ...
  public static void printAVDThreadInfo() {
    octetDumpLock.lockNoHandshake();
    VM.sysWriteln("RVMThread::printOctetThreadInfo() called from Thread#", RVMThread.getCurrentThread().octetThreadID);
    // Scan RVMThread.threads (scan down so we don't miss anything), see comment for threads[]
    for (int i = RVMThread.numThreads-1; i >= 0; i--) {
      Magic.sync();
      RVMThread t = RVMThread.threads[i];
      if (t.isOctetThread()) {
        VM.sysWrite("THREAD ID:", t.octetThreadID);
        VM.sysWrite(" No. of nodes:", t.numberOfNodes);
        VM.sysWrite(" No of regular trans:", t.numberOfRegularTransactions);
        VM.sysWriteln(" No of unary trans:", t.numberOfNodes - t.numberOfRegularTransactions);
      }
    }
    octetDumpLock.unlock();
  }
  
  public static void printOctetThreadInfo(RVMThread t) {
    VM.sysWriteln("Octet Thread ID:", t.octetThreadID);
  }
  
  public void printAVDLogBufferInfo() {
    VM.sysWriteln("Octet thread id:", octetThreadID);
    VM.sysWriteln("Log buffer Reference Index:", logBufferReferenceArrayIndex);
    VM.sysWriteln("Current word array index:", wordArrayBufferIndex);
  }
