...
public final class MainThread extends Thread {
  ...
  public void run() {
    ...
    // AVD: Context: main() method is always supposed to be non-atomic
    if (VM.VerifyAssertions && AVD.isAVDEnabled()) { VM._assert(mainMethod.getMemberRef().asMethodReference().isNonAtomic); }
    if (VM.VerifyAssertions && AVD.isAVDEnabled()) { VM._assert(mainMethod.getStaticContext() == Context.NONTRANS_CONTEXT); }
