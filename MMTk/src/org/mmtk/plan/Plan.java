...
public abstract class Plan implements Constants {
  ...
  // AVD: Separate space for read/write sets
  public static final int ALLOC_AVD_READ_WRITE_SPACE = 9;
...
//  public static final int LOS_SIZE_THRESHOLD = SegregatedFreeListSpace.MAX_CELL_SIZE;
 ...
 // public static final boolean REQUIRES_LOS = VM.activePlan.constraints().requiresLOS();
  ...
  // AVD: Separate space for read/write sets
  public static final LargeObjectSpace avdReadWriteSpace = new LargeObjectSpace("avdReadWriteSpace", VMRequest.create());  
  ...
  // AVD: Separate space for read/write sets
  public static final int AVD_READ_WRITE_SPACE = avdReadWriteSpace.getDescriptor();
  ...
  public void printPreStats() {
    ...
    if (Options.verbose.getValue() >= 2) {
      VM.objectModel.printOctetThreadsInformation();
    }
  }
  ...
  // AVD: Track when harness end is reached. This is helpful since we do not want 
  // to compute newer SCCs and add to the work queue for the Phase 2 thread after 
  // the harness end has been reached.
  public static boolean harnessEndReached = false;
  ...
  public static void harnessEnd()  {
    ...
    // AVD: Indicate the fact that harness end has been reached, so no more work should be added to the 
    // Phase 2 thread
    harnessEndReached = true;
    
    // AVD: Wait till Phase 2 thread completes its "current" work. It seems that 
    // for certain benchmarks such as eclipse6, there could more work generated 
    // (SCCs are detected) even after harness exits.
    if (VM.objectModel.isAVDPhase2Enabled()) {
      while (VM.objectModel.isAVDPhase2ThreadRunning());
    }
  ...
  public int getPagesUsed() {
    return loSpace.reservedPages() + immortalSpace.reservedPages() +
      ...
      + avdReadWriteSpace.reservedPages(); // AVD: Separate space for read/write sets
  }
