...
public abstract class Simple extends Plan implements Constants {
  ...
  // AVD: Added a phase to scan read/write sets
  public static final short AVD_READ_WRITE_SET  = Phase.createSimple("avd-read-write-set", refTypeTime);
  ...
  protected static final short refTypeClosurePhase = Phase.createComplex("refType-closure", null,
      ...
      // AVD: Adding our own phase for scanning read/write sets and RVMThread references as weak references
      // AVD: TODO: Should we invoke this phase later, like in Velodrome?
      Phase.scheduleCollector  (AVD_READ_WRITE_SET),
  ...
  public void collectionPhase(short phaseId) {
    ...
    if (phaseId == PREPARE) {
      ...
      if (!VM.activePlan.global().isCurrentGCNursery()) {
        avdReadWriteSpace.prepare(true);
      }
      return;
    }
    ...
    if (phaseId == RELEASE) {
      ...
      // AVD: Separate space for read/write sets
      if (!VM.activePlan.global().isCurrentGCNursery()) {
        avdReadWriteSpace.release(true);
      }
      return;
    }
