...
public abstract class TraceLocal extends TransitiveClosure implements Constants {
  ...
  public boolean isLive(ObjectReference object) {
    ...
    // AVD: Changes for separate space for read/write sets 
    else if (Plan.avdReadWriteSpace == space)
      return Plan.avdReadWriteSpace.isLive(object);
    else if (space == null) {
      if (VM.VERIFY_ASSERTIONS) {
  ...
  public ObjectReference traceObject(ObjectReference object) {
    ...
    // AVD: Changes for separate read/write space
    if (Space.isInSpace(Plan.AVD_READ_WRITE_SPACE, object)) {  
      object = Plan.avdReadWriteSpace.traceObject(this, object);
      return object;
    }
    if (VM.VERIFY_ASSERTIONS) {
