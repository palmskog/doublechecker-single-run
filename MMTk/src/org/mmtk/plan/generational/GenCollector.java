...
@Uninterruptible public abstract class GenCollector extends StopTheWorldCollector {
  ...
  // AVD: TODO: This will work only for generational collectors. How about Immix?
  public final ObjectReferenceDeque avdOctetThreadsQueue;
  ...
  public GenCollector() {
    ...
    // AVD: Initialize the object reference deque
    avdOctetThreadsQueue = new ObjectReferenceDeque("avdOctetThreadQueue", global().avdOctetThreadsPool);
  ...
  public void collectionPhase(short phaseId, boolean primary) {
    ...
    if (phaseId == Gen.PREPARE) {
      ...
      // AVD: Prepare the shared deque for both full-heap and nursery GCs
      global().avdOctetThreadsPool.prepareNonBlocking();
    ...
    if (phaseId == Gen.RELEASE) {
      ...
      // AVD: Reset threads pool
      global().avdOctetThreadsPool.reset();

      return;
    }
    
    // AVD: Adding our own phase to scan the object references in the read/write sets, 
    // and AVD references in RVMThread
    if (phaseId == Simple.AVD_READ_WRITE_SET) {
      while (!avdOctetThreadsQueue.isEmpty()) {
        ObjectReference objRef = avdOctetThreadsQueue.pop();
        VM.objectModel.processOneOctetThread(objRef);
      }
      if (primary) {
        VM.objectModel.traceAVDGlobalReference();
      }
      return;
    }
