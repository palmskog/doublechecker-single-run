...
public abstract class MutatorContext implements Constants {
  ...
  // AVD: Per-mutator allocator for the separate space for read/write sets
  protected final LargeObjectLocal avdReadWriteSpace = new LargeObjectLocal(Plan.avdReadWriteSpace);
  ...
  public Address alloc(int bytes, int align, int offset, int allocator, int site) {
    ...
    case      Plan.ALLOC_AVD_READ_WRITE_SPACE: return avdReadWriteSpace.alloc(bytes, align, offset);
    default:
  ...
  public void postAlloc(ObjectReference ref, ObjectReference typeRef,
    ...
    // AVD: Separate space for read/write sets
    case   Plan.ALLOC_AVD_READ_WRITE_SPACE: Plan.avdReadWriteSpace.initializeHeader(ref, true); return;
    default:
      VM.assertions.fail("No such allocator");
    }
  }
  ...
  public Allocator getAllocatorFromSpace(Space space) {
    ...
    // AVD: Separate space for read/write sets
    if (space == Plan.avdReadWriteSpace) return avdReadWriteSpace;
  ...
  // AVD: Separate space for allocating read/write sets
  @Inline
  public Address createAVDSpace(int bytes, int align, int offset) {
    return alloc(bytes, align, offset, Plan.ALLOC_AVD_READ_WRITE_SPACE, 0); 
  }
