      ...
      for (;;) {
        ...
        if ((oldState==CLEAR &&
            ...
            (oldState==CLEAR_QUEUED &&
             Magic.attemptInt(this,offset,CLEAR_QUEUED,LOCKED_QUEUED))) {
          break;
          // AVD: Often this is the second enqueue that is happening for allocating resources, violating 
          // Jikes invariant that a thread can be on only one queue at a time, that is why AVD introduces
          // the following additional check to avoid this problem since AVD allocates memory in uninterruptible code
        } else if (me.queuedOn == null &&
                    ((oldState==LOCKED &&
