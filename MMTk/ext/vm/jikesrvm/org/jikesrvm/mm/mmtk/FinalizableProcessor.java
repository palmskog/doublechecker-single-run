...
public final class FinalizableProcessor extends org.mmtk.vm.FinalizableProcessor implements SizeConstants {
  ...
  // AVD: Changes to invoke finalizers for AVDDebugNodes (not for performance runs)
  @UnpreemptibleNoWarn
  public void runFinalizationOnExit() {
    if (AVD.useDebugNodes()) {
      // Iterate over all the entries in the table in FinalizableProcessor and invoke the finalizer    
      for (int i = 0; i < maxIndex; i++) {
        ObjectReference ref = table.get(i).toObjectReference();
        Object o = ref.toObject();
        if (o instanceof AVDDebugNode) {
          try { 
            if (VM.VerifyAssertions) { VM._assert(AVD.methodsAsTransactions()); }
            java.lang.JikesRVMSupport.invokeFinalize(ref.toObject());
          } catch(Throwable e) {
            VM.sysWriteln("AVD: Problem in finalizing AVDDebugNode");
          }
        }
      }    
    }
