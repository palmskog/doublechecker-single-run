...
@Uninterruptible public final class ScanThread implements Constants {
  ...
  private static void scanThread(RVMThread thread, TraceLocal trace,
    ...
    // AVD: Commented out the case for RVMThread.NEW. AVD is adding weak references to even new threads, so it is 
    // required that even those new threads be traced
    // AVD: We also need to scan log buffer reference array for threads that have terminated since the read/write log
    // contents might be getting processed in the PCD
...
//    }
    ...
    // AVD: Construct the deque of RVMThread addresses for each Octet thread, this acts as the producer of the shared
    // deque. We expect that each Octet thread is added to the deque only once.
    // This is executed for both nursery and full heap GCs.
    //if (AVD.logRdWrAccesses() && !org.mmtk.vm.VM.activePlan.global().isCurrentGCNursery()) {
    if (thread.isOctetThread()) {
      org.mmtk.vm.VM.objectModel.addOctetThread(thread.getThreadSlot());
    }
    //}
    
    // AVD: We also need to scan log buffer reference array for threads that have terminated, therefore, we have 
    // commented out the return at the top. In that case, we don't actually need to perform this stuff for "about to 
    // terminate" threads
    if (!thread.getIsAboutToTerminate()) {
  ...
  private void checkReference(Address refaddr, int verbosity) {
    ObjectReference ref = refaddr.loadObjectReference();
    // AVD: Disable checks for JTOC which is not a proper reference
    if (ref.toAddress().EQ(AVD.JTOC_ADDRESS)) {
      return;
    }
